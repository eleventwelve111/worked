...
from dosimetry import analyze_dose_accuracy


# Add this function to the AnalysisReport class
...
# Add this function to generate a dose accuracy report
...
# Add this code block to demonstrate dose accuracy analysis

# Example experimental data (fictional - replace with real measurements)
experimental_data = {
    'distance': [30, 50, 100],
    'angle': [0, 0, 0],
    'dose_rate': [2.5e-3, 9.0e-4, 2.2e-4],  # rem/hr
    'uncertainty': [2.5e-4, 9.0e-5, 2.2e-5]  # 10% uncertainty
}

# Analyze dose accuracy for a specific configuration
energy_kev = 662  # Cs-137
channel_diameter = 1.0  # cm
accuracy_analysis = analysis_report.analyze_dose_accuracy(
    energy_kev, channel_diameter, experimental_data
)

# Generate comprehensive dose accuracy report
accuracy_report = analysis_report.generate_dose_accuracy_report()

# Print the most accurate dose calculation method
print(f"Most accurate dose calculation method for {energy_kev} keV, {channel_diameter} cm: "
      f"{accuracy_analysis['most_accurate_method']}")

# Example of accessing the comparison metrics
if 'experimental_data' in accuracy_analysis:
    for method, metrics in accuracy_analysis['comparison_results'].items():
        if 'mape' in metrics:
            print(f"{method} method MAPE: {metrics['mape']*100:.2f}%")
